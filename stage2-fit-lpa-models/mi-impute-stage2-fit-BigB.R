#  .rs.restartR()
    ...
    pingpong_wd = paste0("D:/Dropbox/Dropbox/Dissertation/ping-pong")
    ...
    for (rep in repseq){
      ...
      if( !(paste0("rep",rep,".csv")%in%list.files(path = pingpong_wd)) ){
        ...
        if(length(unique(tracker_df$nn))!=length(tracker_df$nn)){stop("nn identifier in tracker_df is not unique")}
          ...
          # convert D: to H
          tracker_df$datwd = with(tracker_df, gsub("D:","H:", datwd))
          tracker_df$outwd = with(tracker_df, gsub("D:","H:", outwd))
          tracker_df$rdatawd = with(tracker_df, gsub("D:","H:", rdatawd))
