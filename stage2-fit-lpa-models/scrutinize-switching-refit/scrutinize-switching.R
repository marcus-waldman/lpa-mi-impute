...
# Copy over the clean up batch file
system('xcopy S:\\scrutiny-cleanup.bat H:\\ /Y')
...
for (rep in 1:500){
 if (!(paste0("rep",rep,".csv") %in% list.files("S:/ping-pong"))){
    ...
    # Serve a pong
    write.csv(data.frame(rep = rep, computer = computer_name), file = paste0("S:/ping-pong/rep",rep,".csv"), row.names = FALSE)
    ...
    system(paste0('Bandizip.exe x -y -o:H:\\out-files\\rep',rep,' S:\\out-files\\out-files-rep',rep,'.zip'))
    
    
    # Copy the est-file
    system(paste0('xcopy S:\\est-files\\rep', rep, '.zip H:\\est-files /J /Y'))
    system(paste0('Bandizip.exe x -y -o:H:\\est-files\\rep',rep,' S:\\est-files\\rep',rep,'.zip'))
    ...
    shell.exec('H:\\scrutiny-cleanup.bat')
