# Note that this files simply updates the summaries-combined-results-lpa-mi-impute.RData to include
...
#x = 1    
    classify_df<-
      pblapply(X = 1:500, 
               FUN = function(x){
                 ...
                 out_x$keep = 1
                 out_x$keep[out_x$summary_type=="EAPP"]=0
                 out_x$keep[out_x$summary_type=="pooled, log odds ratio scaled"]=0
                 out_x$keep[out_x$pva==2]=0
                 
                 out_x = out_x %>% filter(keep == 1) %>% 
                 ...
                 # 
                 #out_x = out_x %>% left_join(imputation_procedure_table, by = "Imputation_Procedure")
    ...
    if (!("kl_cprob" %in% names(summaries_combined_df))){
      nrow(summaries_combined_df)
      summaries_combined_df = summaries_combined_df %>% 
              left_join(classify_df %>% select(rep,z,Imputation_Procedure,kl_cprob), by = c("rep","z","Imputation_Procedure"))
      nrow(summaries_combined_df)
      save(summaries_combined_df, file = paste0(results_wd,"/summaries-combined-results-lpa-mi-impute.RData"))
...
#      left_join(conditions_labels, by = "z")
    ...
    # 
    # 
    # classify_agg_df = classify_df %>% 
    #   filter(keep == 1) %>% 
    #   group_by(Imputation_Procedure,summary_type, Mixing,Sample.Size,Separation) %>% 
    #   summarise(kl_cprob = mean(kl_cprob, na.rm = T)) 
    # 
    # ggplot(classify_agg_df %>% filter(startsWith(as.character(Sample.Size), "Large")  ), aes(x = Imputation_Procedure, y = -1*kl_cprob)) + geom_point() + facet_grid(Separation~Mixing)
    # 
    # ggplot(classify_agg_df %>% filter(startsWith(as.character(Sample.Size), "Small")), aes(x = Imputation_Procedure, y = -1*kl_cprob)) + geom_point() + facet_grid(Separation~Mixing)
    # 