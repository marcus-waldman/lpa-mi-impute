#  .rs.restartR()
    ...
    this_file = "mi-impute-test-mgAmelia-Acer.R"
...
#              print("  >> list_imputed constructed")
              ...
              form_lavaan = 'Y1~1
                             ...
                             Y3~~Y2
                             Y2~~Y1'
              obj_lmi = sem.mi(model = form_lavaan, 
                               data = mids2datlist(list_imputed[[1]][[1]][[1]]),
                               group = "subpop",
                               estimator = "ml", information = "observed")
              
              
              # Save the model results
              out_df = obj_lmi %>% summary() %>% data.frame() %>% transform(rep=rep,z=z,pva=1)
