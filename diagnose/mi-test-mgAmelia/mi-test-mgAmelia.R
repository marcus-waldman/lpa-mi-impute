#  .rs.restartR()
    ...
    # Define the imputation alogrithms to compare
      ...
      methods_list$procedure[[1]] = "stratamelia";     methods_list$name[[1]] = "strata-Amelia-weak";      methods_list$args[[1]] = list(m = M, p2s = as.numeric(VerboseImpute), tolerance = 1E-4, empri = 0.001)
#      methods_list$procedure[[2]] = "stratamelia";     methods_list$name[[2]] = "strata-Amelia-weak";      methods_list$args[[2]] = list(m = M, p2s = as.numeric(VerboseImpute), tolerance = 1E-4, empri = 0.005)
#      methods_list$procedure[[3]] = "stratamelia";     methods_list$name[[3]] = "strata-Amelia-moder";     methods_list$args[[3]] = list(m = M, p2s = as.numeric(VerboseImpute), tolerance = 1E-4, empri = 0.01)
#       methods_list$procedure[[1]] = "stratamelia";     methods_list$name[[1]] = "strata-Amelia-strong";    methods_list$args[[1]] = list(m = M, p2s = as.numeric(VerboseImpute), tolerance = 1E-4, empri = 0.05)
#      methods_list$procedure[[5]] = "stratamelia";     methods_list$name[[5]] = "strata-Amelia-verystr";    methods_list$args[[5]] = list(m = M, p2s = as.numeric(VerboseImpute), tolerance = 1E-4, empri = 0.1)
        ...
        sink(file = paste0(lists_wd,"/sink rep", rep, ".txt"))
            ...
            for(z in 1:Ztot){
                 ...
                 print(paste0("########################################################################"))
                  ...
                  #
                   obj_smi_pva10 = sem.mi(model = form_lavaan, 
                                 data = mids2datlist(list_imputed$obj_call$pm1$pva10),
                                 group = "subpop",
                                 estimator = "ml", information = "observed")
                  #
                  # obj_smi_pva11 = sem.mi(model = form_lavaan, 
                  #               data = mids2datlist(list_imputed$obj_call$pm1$pva11),
                  #               group = "subpop",
                  #               estimator = "ml", information = "observed")
                  # 
                  # obj_smi_pva12 = sem.mi(model = form_lavaan, 
                  #                      data = mids2datlist(list_imputed$obj_call$pm1$pva12),
                  #                      group = "subpop",
                  #                      estimator = "ml", information = "observed")
                  # 
                  # obj_smi_pva13 = sem.mi(model = form_lavaan, 
                  #                        data = mids2datlist(list_imputed$obj_call$pm1$pva13),
                  #                        group = "subpop",
                  #                        estimator = "ml", information = "observed")
                  #
                  #obj_smi_pva14 = sem.mi(model = form_lavaan, 
                  #                       data = mids2datlist(list_imputed$obj_call$pm1$pva14),
                  #                       group = "subpop",
                  #                       estimator = "ml", information = "observed")
                 ...
                 vars_keep = c("lhs","op","rhs","group","est","se")
                  ...
                  #save(df_complete, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " complete.rds"))
                 
                  df_pva10 = obj_smi_pva10 %>% summary() %>% data.frame() %>% select(vars_keep) %>% transform(rep=rep,z=z,pva=10,type="Imputed")
                  save(df_pva10, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " pva10.rds"))
                 
                  #df_pva11 = obj_smi_pva11 %>% summary() %>% data.frame() %>% select(vars_keep) %>% transform(rep=rep,z=z,pva=11,type="Imputed")
                  #save(df_pva11, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " pva11.rds"))
                  
                  #df_pva12 = obj_smi_pva12 %>% summary() %>% data.frame() %>% select(vars_keep) %>% transform(rep=rep,z=z,pva=12,type="Imputed")
                  #save(df_pva12, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " pva12.rds"))
                  
                  #df_pva13 = obj_smi_pva13 %>% summary() %>% data.frame() %>% select(vars_keep) %>% transform(rep=rep,z=z,pva=13,type="Imputed")
                  #save(df_pva13, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " pva13.rds"))
                  
                  #df_pva14 = obj_smi_pva14 %>% summary() %>% data.frame() %>% select(vars_keep) %>% transform(rep=rep,z=z,pva=14,type="Imputed")
                  #save(df_pva14, file = paste0(lists_wd,"/result-test-mgAmelia rep",rep, " z",z, " pva14.rds"))
                  
                  print("  >> .rds files saved")
