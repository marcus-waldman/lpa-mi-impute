    ...
    # Gaby
      ...
      datfiles_dir = "H:" #Stores Mplus-read dat files
      Rdata_dir = "H:"
    ...
    N_vec = c(300)               #  Sample sizes within each replication (small - 300, large - 1200)
    ...
    MD_vec = c(2.5)               #  Mahalanobis disance separating classes (low - 2.87, high = 3.70)
    pi_list = list(               #  Mixing and (by proxy) the number of classes, K (maximum of 3)
      A = rep(1/3,3)
    )
    rho_YX_vec = c(0.4)            #  Strength of missing data correlates, correlation
    ...
    for (rep in 1:Replications){
      for(z in 1:Ztot){
        dir.create(paste0(datfiles_wd_rep_vec[rep],"/z",z,"/Complete data"), recursive = TRUE)
        for (i in 1:length(pctmiss_vec)){
          dir.create(paste0(datfiles_wd_rep_vec[rep],"/z",z,"/Observed data/pm", pm_vec[i]), recursive = TRUE)
        }
      }
    }
    ...
    ######################################################################################################################
    set.seed(RndSeed)
    seeds_vec = .Random.seed[1:Replications]
    ...
    rep = 1
    z = 1
#    sink(file = paste0(lists_wd, "/sink-stage0 rep", rep,".txt"))
#    for(rep in 1:Replications){
      RndSeed_rep = seeds_vec[rep]
      set.seed(RndSeed_rep)
#      for(z in 1:Ztot){
        ...
        save(list = c("list_complete"), 
             file = paste0(lists_wd, "/list-complete rep", rep, " z",z,".RData"))
        print("  >> list_complete saved")
        
        # Observed data
        for(i in 1:pctmiss_vec){
          list_observed = lpa.mi.src::get_obs_data(z=z, 
                                                   ...
                                                   temp_wd_rep_vec = datfiles_wd_rep_vec,
                                                   pm_vec = pm_vec[i])
           save(list = c("list_observed"), 
               file = paste0(lists_wd, "/list-observed rep", rep, " z",z," pm", pm_vec[i],".RData"))
            print(paste0("  >> list_observed saved for pm", pm_vec[i]))
        }
